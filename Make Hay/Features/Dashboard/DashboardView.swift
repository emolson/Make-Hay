...
#Preview("Progress - 50%") {
    ...
    return DashboardView(viewModel: DashboardViewModel(healthService: mock))
}
...
#Preview("Loading State") {
    let mock = MockHealthService()
    let viewModel = DashboardViewModel(healthService: mock)
    return DashboardView(viewModel: viewModel)
}

#Preview("Error State") {
    let viewModel = DashboardViewModel(healthService: ErrorThrowingMockHealthService())
    return DashboardView(viewModel: viewModel)
}
